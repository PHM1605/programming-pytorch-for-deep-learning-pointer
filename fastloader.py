...
# --- this is good because we add all images at once ---
def add_noise_gpu(tensor, device):
  random_noise = torch.rand_like(tensor).to(device)
  return tensor.add_(random_noise)

def train_good(model, optimizer, loss_fn, train_loader, val_loader, epochs=20, device="cuda"):
  model.to(device)
  for epoch in range(epochs):
    ...
    for images, targets in train_loader:
      ...
      images = add_noise_gpu(images, device)
...
train_good(model, optimizer, criterion, train_data_loader, train_data_loader, epochs=10)
